...
{
    public class DBOleDbSchemaProvider : DBSchemaProvider
    {

        public DBOleDbSchemaProvider(DBDatabase database, DBDatabaseProperties properties)
        ...
        protected int LoadTableRefs(DbConnection con, List<DBSchemaItemRef> addtocollection)
        {
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                if (stype == "TABLE" || stype == "ACCESS TABLE")
                {
        ...
        protected int LoadSprocRefs(DbConnection con, List<DBSchemaItemRef> addtocollection)
        {
            ...
            DataColumn catalog = dt.Columns["PROCEDURE_CATALOG"];
            DataColumn schema = dt.Columns["PROCEDURE_SCHEMA"];
            DataColumn name = dt.Columns["PROCEDURE_NAME"];
            DataColumn type = dt.Columns["PROCEDURE_TYPE"];
        ...
        protected override DBSchemaTable LoadATable(DbConnection con, DBSchemaItemRef forRef)
        {
            return base.LoadATable(con, forRef);
        }
