...
namespace Scryber.OpenType
{
    public abstract class TTFTableFactory
    {
        ...
        public TTFTableFactory(bool throwonnotfound, IEnumerable<string> required)
        {
        ...
        {
            TTFDirectory dir = list[tname];

            return ReadDirectory(dir, list, reader);
        }

        public TTFTable ReadDirectory(TTFDirectory directory, TTFFile file, BigEndianReader reader)
        {
            return this.ReadDirectory(directory, file.Directories, reader);
        }

        public TTFTable ReadDirectory(TTFDirectory directory, TTFDirectoryList list, BigEndianReader reader)
        {
    ...
    public class TTFOpenTypeTableFactory : TTFTableFactory
    {
        ...
        private TTFTable ReadCMAPTable(uint length, TTFDirectoryList list, BigEndianReader reader)
        {
            ...
            for (int i = 0; i < num; i++)
            {
                ...
                rec.Encoding = (CharacterEncodings)enc;
                rec.Platform = (CharacterPlatforms)plat;
        ...
        private TTFTable ReadHorizontalMetrics(uint length, TTFDirectoryList list, BigEndianReader reader)
        {
            ...
            if (hhead.Table == null)
            {
                TTFTable tbl = this.ReadDirectory(hhead, list, reader);
                if(tbl == null)
                    throw new ArgumentNullException("No horizontal header (hhea) has been specified in the font file, this is needed to load the hroizontal metrics");
                hhead.SetTable(tbl);
            }
            ...
            if (os2.Table == null)
            {
                TTFTable tbl = this.ReadDirectory(os2, list, reader);
                if (tbl == null)
