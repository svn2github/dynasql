...
namespace Scryber
{
    ...
    public abstract class PDFWriter : IDisposable
    {
        ...
        /// <summary>
        /// Gets the XRefTable that holds all references and their underlying stream positions
        /// </summary>
        ...
        #region Stream InnerStream {get;} + TextWriter InnerWriter {get;}

        private PDFStream _inners;
        ...
        protected PDFStream CurrentStream
        {
            get
            {
                if (this._stack.Count == 0)
                    throw new ArgumentOutOfRangeException("There is no current object in the document");
        ...
        protected PDFWriter(Stream stream, PDFXRefTable table, PDFTraceLog log)
        {
            if (null == stream)
                throw new ArgumentNullException("stream");
            if (stream.CanWrite == false)
                throw new ArgumentException(Errors.CannotWriteToThisStream, "stream");
            if (null == table)
                throw new ArgumentNullException("table");

            this._inners = new PDFStream(stream, this.DefaultStreamFilters, null, false);
            this._xreftable = table;
            this.Stack.Push(_inners);
            if (null == _log)
                _log = new Scryber.DoNothingTraceLog();
            this._log = log;
        }
        ...
        /// </summary>
        protected virtual void ReleaseIndirectObject()
        {
            this.Stack.Pop();
        ...
        #region OpenDocument() + CloseDocument()
        ...
        public abstract void Close(string[] documentid);

        #endregion

        public PDFObjectRef BeginPage(int pgIndex)
        {
        ...
        public void EndPage(int pgIndex)
        {
            this.EndObject();
        }
        ...
        public abstract void WriteStringLiteral(string value);
