...
namespace Scryber.Data
{
    public class PDFParsableTemplateGenerator : IPDFTemplate, IPDFTemplateGenerator
    {
        ...
        public PDFParsableTemplateGenerator(IPDFComponent owner, string xmlcontent, System.Xml.XmlNamespaceManager namespaces)
            : this()
        {
            InitTemplate(owner, xmlcontent, namespaces);
        }

        public void InitTemplate(IPDFComponent owner, string xmlcontent, System.Xml.XmlNamespaceManager namespaces)
        {
            if (null == owner)
        ...
        public IEnumerable<IPDFComponent> Instantiate(int index)
        {
            if (!_initialised)
            ...
            using (System.IO.StringReader sr = new System.IO.StringReader(this._toparse))
            {
                ...
                IPDFRemoteComponent remote = this.GetRemoteComponent();
                IPDFDocument doc = this._owner.Document;
                IPDFComponent comp;
                ...
                List<PDFComponent> all = new List<PDFComponent>(1);
        ...
        protected IPDFRemoteComponent GetRemoteComponent()
        {
            IPDFComponent comp = this._owner;
            while (null != comp)
            {
